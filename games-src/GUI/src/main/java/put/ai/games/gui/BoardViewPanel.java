...
public class BoardViewPanel extends JPanel {
    ...
    private void drawPlaceMove(Graphics g, PlaceMove pm, int d) {
        ...
        g.drawRect(boardToScreenX(pm.getX(), d), boardToScreenY(pm.getY(), d), d, d);
    }
    ...
    private void drawSkipMove(Graphics g, SkipMove pm, int d) {
        ...
        g.drawRect(boardToScreenX(pos.getX(), d), boardToScreenY(pos.getY(), d), d, d);
    }
    ...
    private void drawArrow(Direction dir, MoveMove mm, int d, Graphics g) {
        ...
        if (dir != Direction.UNDEFINED) {
            poly = new Polygon();
            for (double[] p : arrow) {
                int x = boardToScreenX(mm.getDstX(), d) + d / 2;
                int y = boardToScreenX(mm.getDstY(), d) + d / 2;
    ...
    private int boardToScreenX(int x, int d) {
        return marginX + x * d;
    }

    private int boardToScreenY(int y, int d) {
        return marginY + y * d;
    }
    ...
    public void paint(Graphics g) {
        ...
        int boardSize = board.getSize() * d;
        for (int i = 0; i <= board.getSize(); ++i) {
            int x = boardToScreenX(i, d);
            int y = boardToScreenY(i, d);
            g.drawLine(x, marginY, x, marginY + boardSize);
            g.drawLine(marginX, y, marginX + board.getSize() * d, y);
        }
        {
            int textMarginX = (int) (0.2 * marginX);
            int textMarginY = (int) (0.2 * marginY);
            int targetWidth = marginX; // - 2 * textMarginX;
            int targetHeight = marginY; // - 2 * textMarginY;
            Font baseFont = g.getFont();
            for (int s = 0; s < 100; ++s) {
                Font f = baseFont.deriveFont((float) s);
                FontMetrics metrics = g.getFontMetrics(f);
                // M tends to be one of the wider characters in the latin alphabet. Using getMaxCharBounds gives a useless result
                double w = metrics.charWidth('M');
                // not using getHeight since we are not interested in the leading
            ...
            for (int i = 0; i < board.getSize(); ++i) {
                ...
                g.drawString(s, (int) (x + d / 2 - bounds.getWidth() / 2), marginY - textMarginY);
            }
            for (int i = 0; i < board.getSize(); ++i) {
                int y = boardToScreenY(board.getSize() - i - 1, d);
                String s = Integer.toString(i + 1);
                Rectangle2D bounds = metrics.getStringBounds(s, g);
                g.drawString(s, textMarginX, (int) (y + d / 2 - bounds.getY() - bounds.getHeight() / 2));
            }
        }
        g.setColor(Color.LIGHT_GRAY);
        for (int x = 0; x < board.getSize(); ++x) {
            int sx = boardToScreenX(x, d) + 1;
            for (int y = 0; y < board.getSize(); ++y) {
                int sy = boardToScreenY(y, d) + 1;
                g.fillRect(sx, sy, d - 1, d - 1);
                g.drawImage(getImage(x, y), sx, sy, null);
            }
        }
        {
            Point pos = getSkipButtonPosition();
            int x = boardToScreenX(pos.getX(), d) + 1;
            int y = boardToScreenY(pos.getY(), d) + 1;
            g.fillRect(x, y, d - 1, d - 1);
            g.drawImage(scaledSkip, x, y, null);
        }
    ...
    private void vertLine(Polygon p, int x, int y1, int y2, int d) {
        int w = d / 10;
        assert y1 < y2;
        x = boardToScreenX(x, d);
        y1 = boardToScreenY(y1, d);
        y2 = boardToScreenY(y2, d);
        p.addPoint(x + d / 2 + w, y1 + d / 2 - w);
        p.addPoint(x + d / 2 - w, y1 + d / 2 - w);
        p.addPoint(x + d / 2 - w, y2 + d / 2 + w);
        p.addPoint(x + d / 2 + w, y2 + d / 2 + w);
    }
    ...
    private void horizLine(Polygon p, int x1, int x2, int y, int d) {
        int w = d / 10;
        assert x1 < x2;
        x1 = boardToScreenX(x1, d);
        x2 = boardToScreenX(x2, d);
        y = boardToScreenY(y, d);
        p.addPoint(x1 + d / 2 - w, y + d / 2 + w);
        p.addPoint(x1 + d / 2 - w, y + d / 2 - w);
        p.addPoint(x2 + d / 2 + w, y + d / 2 - w);
        p.addPoint(x2 + d / 2 + w, y + d / 2 + w);
    }
    ...
    private class TranslatingMouseListener implements MouseListener {
        ...
        private MouseEvent translate(MouseEvent e) {
            ...
            int x = (e.getX() - marginX) / getSide();
            int y = (e.getY() - marginY) / getSide();
